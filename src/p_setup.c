...
#define gNd2            0x32644E67  // figgi -- suppport for new GL_VERT
#define gNd3            0x33644E67  //          format v2.0
...
typedef struct {
  ...
  unsigned short partner; // corresponding partner seg, or -1 on one-sided
} glseg_t;                // walls
...
static int P_CheckForZDoomUncompressedNodes(int lumpnum, int gl_lumpnum) {
  ...
  if (!memcmp(data, "XNOD", 4)) {
    D_Msg(MSG_INFO,
      "P_CheckForZDoomUncompressedNodes: ZDoom uncompressed normal nodes "
      "are detected\n"
    );
    result = true;
  }
  ...
  if ((gl_lumpnum > lumpnum) && (!forceOldBsp) &&
    ...
    if (*(const int *)data == gNd2) {
      ...
      if (*(const int *)data == gNd3) {
        ...
        nodesVersion = gNd2;
        D_Msg(MSG_DEBUG, "P_GetNodesVersion: found version 2 nodes\n");
      }
    }
    ...
    if (nodesVersion == 0 && ver != -1) {
      D_Msg(MSG_DEBUG, "P_GetNodesVersion: found version %d nodes\n", ver);
      D_Msg(MSG_DEBUG,
    ...
    D_Msg(MSG_DEBUG,"P_GetNodesVersion: using normal BSP nodes\n");
...
static void P_LoadSegs (int lump) {
  ...
  for (i = 0; i < numsegs; i++) {
    ...
    if ((unsigned)ldef->sidenum[side] >= (unsigned)numsides) {
      I_Error(
        "P_LoadSegs: linedef %d for seg %d references a non-existent "
        "sidedef %d",
        linedef,
        i,
        (unsigned)ldef->sidenum[side]
      );
    }
    ...
    if (ldef->sidenum[side] != NO_INDEX) {
      ...
      li->frontsector = 0;
      D_Msg(MSG_WARN, "P_LoadSegs: front of seg %i has no sidedef\n", i);
    }

    if (ldef->flags & ML_TWOSIDED) {
      int sidenum = ldef->sidenum[side ^ 1];

      if (sidenum == NO_INDEX) // this is wrong
    ...
    if (v1 >= numvertexes || v2 >= numvertexes) {
      char str[200] = "P_LoadSegs: compatibility loss - seg %d references "
                      "a non-existent vertex %d\n";
      ...
      if (demorecording) {
        I_Error(
          ...
          i,
          (v1 >= numvertexes ? v1 : v2)
        );
      }

      if (v1 >= numvertexes)
        D_Msg(MSG_WARN, str, i, v1);

      if (v2 >= numvertexes)
        D_Msg(MSG_WARN, str, i, v2);
...
static void P_LoadSegs_V4(int lump) {
  ...
  for (i = 0; i < numsegs; i++) {
    ...
    if (side != 0 && side != 1) {
      D_Msg(MSG_WARN,
        "P_LoadSegs_V4: seg %d contains wrong side index %d. Replaced with "
        "1.\n",
        i,
        side
      );
    ...
    if ((unsigned)ldef->sidenum[side] >= (unsigned)numsides) {
      I_Error(
        "P_LoadSegs_V4: linedef %d for seg %d references a non-existent "
        "sidedef %d",
        linedef,
        i,
        (unsigned)ldef->sidenum[side]
    ...
    if (ldef->sidenum[side] != NO_INDEX) {
      ...
      li->frontsector = 0;
      D_Msg(MSG_WARN, "P_LoadSegs_V4: front of seg %i has no sidedef\n", i);
    }
    ...
    // http://www.doomworld.com/idgames/index.php?id=12647
    if (v1 >= numvertexes || v2 >= numvertexes) {
      char str[200] = "P_LoadSegs_V4: compatibility loss - seg %d references "
                      "a non-existent vertex %d\n";
...
static void P_LoadGLSegs(int lump) {
  ...
  for (i = 0; i < numsegs; i++) { // check for gl-vertices
...
static void P_LoadSubsectors(int lump) {
  /*
   * cph 2006/07/29 - make data a const mapsubsector_t *, so the loop below is
   * simpler & gives no constness warnings
   */
  const mapsubsector_t *data;
  int  i;

  numsubsectors = W_LumpLength (lump) / sizeof(mapsubsector_t);
  subsectors = calloc_IfSameLevel(
    subsectors, numsubsectors, sizeof(subsector_t)
  );
...
static void P_LoadZSegs(const unsigned char *data) {
  ...
  for (i = 0; i < numsegs; i++) {
    ...
    if ((unsigned)ldef->sidenum[side] >= (unsigned)numsides) {
      I_Error(
        "P_LoadZSegs: linedef %d for seg %d references a non-existent "
        "sidedef %d",
        linedef,
        i,
        (unsigned)ldef->sidenum[side]
    ...
    * referencing the back of a 1S line */
    if (ldef->sidenum[side] != NO_INDEX) {
      ...
      li->frontsector = 0;
      D_Msg(MSG_WARN, "P_LoadZSegs: front of seg %i has no sidedef\n", i);
    }
    ...
    /*
    li->angle = (int)((float)atan2(
      li->v2->y - li->v1->y,li->v2->x - li->v1->x) * (ANG180 / M_PI)
    );
    */
  }
}
...
static void P_LoadZNodes(int lump, int glnodes) {
  ...
  if (!samelevel) {
    ...
    int size = newVerts * (sizeof(newvertarray[0].x) +
  ...
  for (i =0; i < numlines; i++) {
...

    {
      ...
      if ((ld->sidenum[1] == NO_INDEX) && (ld->flags & ML_TWOSIDED)) {
        ...
        // cph - print a warning about the bug
        D_Msg(MSG_WARN,
          "P_LoadLineDefs: linedef %d has two-sided flag set, but no second "
          "sidedef\n",
          i
        );
      }
...
static void P_CreateBlockMap(void) {
  ...
  //jff 10/12/98: +1 needed for map exactly 1 cell
      ...
      for (j = 0; j < nrows; j++) {
        ...
        if (xp == 0) { // intersection at a corner
          if (sneg) { //   \ - blocks x,y-, x-,y
            ...
            if (xb > 0 && minx < x) {
              AddBlockLine(
                blocklists, blockcount, blockdone, ncols * j + xb - 1, i
              );
            }
          }
  ...
  for(y = 0; y < bmapheight; y++) {
    for(x = 0; x < bmapwidth; x++) {
      ...
      if (blockoffset >= maxoffs) {
        D_Msg(MSG_ERROR, "P_VerifyBlockMap: block offset overflow\n");
        return false;
      }
      ...
      if (offset < 4 || offset >= count) {
        D_Msg(MSG_ERROR, "P_VerifyBlockMap: list offset overflow\n");
        return false;
      }
      ...
      for (tmplist = list; ; tmplist++) {
        ...
        if (tmplist >= maxoffs) {
          D_Msg(MSG_ERROR, "P_VerifyBlockMap: open blocklist\n");
          return false;
        }
      ...
      for (tmplist = list; *tmplist != -1; tmplist++) {
        if (*tmplist < 0 || *tmplist >= numlines) {
          D_Msg(MSG_ERROR, "P_VerifyBlockMap: index >= numlines\n");
          return false;
        }
      }
    }
...
static void P_LoadBlockMap(int lump) {
  ...
  if (blockmapxneg != -257 || blockmapyneg != -257) {
    D_Msg(MSG_WARN,
      "P_LoadBlockMap: This map uses a large blockmap which may cause "
      "no-clipping bugs. Toggle the 'Fix clipping problems in large levels' "
      "option in the 'Compatibility with common mapping errors' menu in "
      "order to activate a fix. That fix won't be applied during demo "
...
static int P_GroupLines(void) {
  ...
  for (i = 0, sector = sectors; i < numsectors; i++, sector++) {
    // cph - For convenience, so I can use the old code unchanged
    fixed_t *bbox = (void *)sector->blockbox;
    int block;
...
void P_InitSubsectorsLines(void) {
  ...
  for (num = 0; num < numsubsectors; num++) {
    seg_t *seg;
    const seg_t *seg_last = segs +
                            subsectors[num].firstline +
                            subsectors[num].numlines;
...
void P_SetupLevel(int episode, int map, int playermask, skill_e skill) {
  ...
  // Most of level's structures now are allocated with PU_STATIC instead of
  // PU_LEVEL It is important for OpenGL, because in case of the same data in
  // memory we can skip recalculation of much stuff

  samelevel = (map == current_map) &&
  ...
  else if (P_CheckForZDoomUncompressedNodes(lumpnum, gl_lumpnum)) {
...
#ifdef GL_DOOM
  map_subsectors = calloc_IfSameLevel(
  ...
  if (V_GetMode() == VID_MODEGL) {
    ...
    if (!doSkip) // proff 11/99: calculate all OpenGL specific tables etc.
      gld_PreprocessLevel();
  }
#endif
  //e6y
