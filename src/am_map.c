...
const char *map_things_appearance_list[map_things_appearance_max] =
{
  ...
  "icons"
#endif /* if defined(HAVE_LIBSDL_IMAGE) && defined(GL_DOOM) */
};
...
#define F_PANINC (gamekeydown[key_speed] ? map_scroll_speed * \
  2 : map_scroll_speed)
...
// precise versions
#define MTOF_F(x) (((float)(x) * scale_mtof) / (float)FRACUNIT / \
  (float)FRACUNIT)
...
static fixed_t mtof_zoommul;    // how far the window zooms each tic (map
                                // coords)
static fixed_t ftom_zoommul;    // how far the window zooms each tic (fb coords)
static fixed_t curr_mtof_zoommul;

static fixed_t m_x, m_y;        // LL x,y window location on the map (map
                                // coords)
static fixed_t m_x2, m_y2;      // UR x,y window location on the map (map
                                // coords)
...
int markpointnum = 0;           // next point to be assigned (also number of
                                // points now)
...
void AM_SetPosition(void) {
  if (automapmode & am_overlay) {
    ...
    // default
    f_x = f_y = 0;
    f_w = SCREENWIDTH;                    // killough 2/7/98: get rid of finit_
                                          // vars
    f_h = SCREENHEIGHT - ST_SCALED_HEIGHT;// to allow runtime setting of
                                          // width/height
  }
}
...
static void AM_initVariables(void) {
  ...
  if (!player) {
    ...
    if (!found_player) {
      I_Error("AM_initVariables: No players");
    }
  }
...
void AM_rotatePoint(mpoint_t *p) {
  ...
  if (am_frame.precise) {
    ...
    f     = (p->fx * am_frame.cos_f) - (p->fy * am_frame.sin_f) +
      am_frame.centerx_f;
    p->fy = (p->fx * am_frame.sin_f) + (p->fy * am_frame.cos_f) +
      am_frame.centery_f;
    p->fx = f;
  }
  ...
  tmpx =
    FixedMul(p->x,
    am_frame.cos) - FixedMul(p->y, am_frame.sin) + am_frame.centerx;
  p->y =
    FixedMul(p->x,
    am_frame.sin) + FixedMul(p->y, am_frame.cos) + am_frame.centery;
  p->x = tmpx;
}
...
#define DOOUTCODE(oc, mx, my) \
  ...
  while (outcode1 | outcode2) {
    ...
    if (outside & TOP) {
      ...
      dx = fl->b.x - fl->a.x;
      tmp.y = fl->a.y +
        (fixed_t)(((int64_t)dy * (f_x + f_w - 1 - fl->a.x)) / dx);
      tmp.x = f_x + f_w - 1;
      if (am_frame.precise) {
...
static void AM_drawGrid(int color) {
  ...
  minlen = M_DoubleToInt(
    sqrt((float)m_w * (float)m_w + (float)m_h * (float)m_h)
  );
  extx = (minlen - m_w) / 2;
  exty = (minlen - m_h) / 2;
...
static inline bool draw_keyed_doors(void) {
  return mapcolor_bdor || mapcolor_ydor || mapcolor_rdor;
}

static inline bool line_is_non_secret_keyed_door(line_t *line, int *color) {
  int c;
...
static inline bool line_is_exit(line_t *line) {
  return (
    line->special == 11  ||
    line->special == 52  ||
    line->special == 197 ||
    line->special == 51  ||
    line->special == 124 ||
    line->special == 198 /* exit line */
  );
}

static inline bool line_is_one_sided_secret(line_t *line) {
  return ((
...
static inline bool line_is_non_secret_teleport(line_t *line) {
  return (
...
static inline bool line_is_non_secret_closed_door(line_t *line) {
  return (
...
static inline bool line_is_computer_map_visible(line_t *line) {
  return (
...
static void AM_drawWalls(void) {
  ...
  for (i = 0; i < numlines; i++) {
    ...
    if (ddt_cheating || (line->flags & ML_MAPPED)) {
      ...
      if (draw_keyed_doors() && line_is_non_secret_keyed_door(line, &amd)) {
      ...
      if (mapcolor_exit && line_is_exit(line)) {
      ...
      if (!line->backsector) {
        ...
        if (mapcolor_tele && line_is_non_secret_teleport(line)) {
          AM_drawMline(&l, mapcolor_tele);
        }
        else if (line->flags & ML_SECRET) {  // secret door
          AM_drawMline(&l, mapcolor_wall);      // wall color
        }
        else if (mapcolor_clsd && line_is_non_secret_closed_door(line)) {
          AM_drawMline(&l, mapcolor_clsd);      // non-secret closed door
        }
        else if (mapcolor_secr && line_is_non_secret_closed_door(line)) {
    ...
    else if (plr->powers[pw_allmap] && line_is_computer_map_visible(line)) {
...
static void AM_drawPlayers(void) {
  ...
  if (map_things_appearance == map_things_appearance_scaled) {
    scale = (
      BETWEEN(4 << FRACBITS, 256 << FRACBITS, plr->mo->radius) >> FRACTOMAPBITS
    );
  }
  else {
    scale = 16 << MAPBITS;
  ...
  if (!netgame) {
    ...
    if (ddt_cheating) {
      AM_drawLineCharacter(cheat_player_arrow,
        NUMCHEATPLYRLINES,
        scale,
        viewangle,
        mapcolor_sngl,
      ...
      AM_drawLineCharacter(player_arrow,
        NUMPLYRLINES,
        scale,
        viewangle,
        mapcolor_sngl,
...
static void AM_ProcessNiceThing(mobj_t *mobj,
  ...
  if (mobj->player) {
    ...
    if (p->id == 0) {
      color = vanilla_mapplayer_colors[0];
    }
    else {
      color = vanilla_mapplayer_colors[(p->id - 1) % VANILLA_MAXPLAYERS];
    }
  ...
  ang = (rotate ? angle : 0) +
    ((automapmode & am_rotate) ? ANG90 - viewangle : 0);
  ...
  for (i = 0; i < numsectors; i++) {
    ...
    for (pass = 0; pass < 2; pass += (enemies ? 1 : 2)) {
      ...
      while (t) { // for all things in that sector
        ...
        if (map_things_appearance == map_things_appearance_scaled) {
          scale = (
            BETWEEN(4 << FRACBITS, 256 << FRACBITS, t->radius) >> FRACTOMAPBITS
          ); // * 16 / 20;
        }
        else {
          scale = 16 << MAPBITS;
...
static void AM_drawMarks(void) {
  ...
  for (i = 0; i < markpointnum; i++) { // killough 2/22/98: remove automap mark
                                       // limit
    if (markpoints[i].x != -1) {
      int k, w;
      mpoint_t p;
      ...
      if (am_frame.precise) {
        p.fx = CXMTOF_F(p.fx) - (float)markpoints[i].w * SCREENWIDTH / 320.0f /
          2.0f;
        p.fy = CYMTOF_F(p.fy) - (float)markpoints[i].h * SCREENHEIGHT / 200.0f /
          2.0f;
      }
