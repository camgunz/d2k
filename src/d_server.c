...
int playerjoingame[MAXPLAYERS];
int playerleftgame[MAXPLAYERS];
obuf_t *clients = NULL;
...
void verbose_printf(int level, const char *fmt, ...) {
  va_list argptr;
...
byte def_game_options[GAME_OPTIONS_SIZE] = { // cf g_game.c:G_WriteOptions()
  1,          /* monsters remember */
  1,          /* friction */
  0,          /* weapon recoil */
  1,          /* pushers */
...
const char* gameopt_config_names[] = {
  ...
  "weapon_recoil",
  NOSUCH,                         /* pushers */
  NOSUCH,                         /* reserved/unused */
  "player_bobbing",
  NOSUCH,                         /* respawn monsters */
  NOSUCH,                         /* fast monsters */
  NOSUCH,                         /* nomonsters */
  "demo_insurance",
...
void NORETURN sig_handler(int signum) {
  ...
  exit(1); // Any signal is fatal
}

void doexit(void) {
  packet_header_t packet;

  packet_set(&packet, PKT_DOWN, 0); // Send "downed" packet
  BroadcastPacket(&packet, sizeof(packet));
}

void I_InitSockets(uint16_t port) {
  I_InitNetwork();
  if (!I_Listen(0, port))
    I_Error("I_InitSockets: Failed to listen on port :%u.\n", port);
}
  ...
  while (!feof(fp)) {
    ...
    if (fscanf(fp, "%79s %99[^\n]\n", def, strparm) == 2) {
      ...
      if (!strcmp(def, "default_skill")) {
        verbose_printf(1, "config file sets default_skill to %d\n", v);
        sp->skill = v - 1;
      }
      else if (!strcmp(def, "default_compatibility_level")) {
        verbose_printf(1, "config file sets compatibility_level to %d\n", v);
        if (v == -1)
        ...
        for (i = 0; i < num_gameopts; i++) {
          if (!!strcmp(gameopt_config_names[i], def))
            continue;
          verbose_printf(1, "config file sets %s to %d\n", def, v);
          gameopt[i] = v;
        }
      }
    }
...
int main(int argc, char **argv) {
  ...
  struct setup_packet_s setupinfo = {
    2, 0, 1, 1, 1, 0, best_compatibility, 0, 0
  };
  char **wadname = NULL;
  char **wadget = NULL;
  int numwads = 0;
...

  {
    ...
    while (1) {
      ...
      while ((len = I_GetPacket(packet, 10000))) {
        verbose_printf(3, "Received packet:");
        switch (packet->type) {
          case PKT_INIT:
            if (!ingame) {
              int n;
...

              {
                ...
                I_SendPacketTo(
                  packet,
                  sizeof(*packet) + sizeof(setupinfo) + extrabytes,
                  remoteaddr + n
                );
                I_uSleep(10000);
                I_SendPacketTo(
          ...
          case PKT_TICC:
          {
            ...
            verbose_printf(3, "tics %ld - %ld from %d\n",
         ...
         case PKT_RETRANS:
         {
           ...
           verbose_printf(3, "%d requests resend from %ld\n",
             from, ptic(packet)
           );
         ...
         case PKT_QUIT:
         {
           ...
           if (!ingame && playerstate[from] != pc_unused) {
             ...
             --curplayers;
             verbose_printf(1, "%d quits at %ld (%d left)\n",
         ...
         case PKT_WAD:
         {
           ...
           verbose_printf(1, "Request for %s ", name);
           for (i = 0; i < numwads; i++) {
      ...
      if (ingame) { // Run some tics
        ...
        verbose_printf(2, "%d new tics can be run\n", lowtic - exectics);
        ...
        for (i = 0; i < MAXPLAYERS; i++) {
          if (playerstate[i] == pc_playing) {
...

            {
              ...
              *p++ = tics;
              verbose_printf(2, "sending %d tics to %dn", tics, i);
                ...
                for (j = 0; j < MAXPLAYERS; j++) {
                  if ((playerjoingame[j] <= remoteticto[i]) &&
                    ...
                    *p++ = j;
                    memcpy(
                      p,
                      &netcmds[j][remoteticto[i] % BACKUPTICS],
                      sizeof(ticcmd_t)
                    );
                    p += sizeof(ticcmd_t);
                    playersthistic++;
            ...
            if (remoteticfrom[i] == remoteticto[i]) {
              ...
              backoffcounter[i] += remoteticfrom[i] - remoteticto[i] - 1;

              if (backoffcounter[i] > 35) {
                ...
                backoffcounter[i] = 0;
                verbose_printf(1, "telling client %d to back off\n", i);
                free(packet);
              }
            }
          }
