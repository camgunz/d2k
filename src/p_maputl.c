...
void P_UnsetThingPosition(mobj_t *thing) {
  /*
   * invisible things don't need to be in sector list unlink from subsector
   *
  ...
  if (!(thing->flags & MF_NOSECTOR)) {
    ...
    // phares 3/14/98
    //
    // Save the sector list pointed to by touching_sectorlist.  In
    // P_SetThingPosition, we'll keep any nodes that represent sectors the
    // Thing still touches. We'll add new ones then, and delete any nodes for
    // sectors the Thing has vacated. Then we'll put it back into
    // touching_sectorlist. It's done this way to avoid a lot of
    // deleting/creating for nodes, when most of the time you just get back
    // what you deleted anyway.
    //
    // If this Thing is being removed entirely, then the calling routine will
    // clear out the nodes in sector_list.
   ...
   * unlinking. Old method required computing head node based on position at
   * time of unlinking, assuming it was the same position as during linking.
   */
...
//
// killough 5/3/98: reformatted, cleaned up
void P_SetThingPosition(mobj_t *thing) { // link into subsector
  subsector_t *ss = thing->subsector = R_PointInSubsector(thing->x, thing->y);

  // invisible things don't go into the sector links

  // killough 8/11/98: simpler scheme using pointer-to-pointer prev pointers,
  ...
  if (!(thing->flags & MF_NOSECTOR)) {
    ...
    *link = thing;
    ...
    // phares 3/16/98
    //
    // If sector_list isn't NULL, it has a collection of sector nodes that were
    // just removed from this Thing.

    // Collect the sectors the object will live in by looking at the existing
    // sector_list and adding new nodes and deleting obsolete ones.

    // When a node is deleted, its sector links (the links starting at
    // sector_t->touching_thinglist) are broken. When a node is added, new
    // sector links are created.
