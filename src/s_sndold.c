...
static int get_channel(void *origin, sfxinfo_t *sfxinfo, int is_pickup) {
                               ...
                               int *vol, int *sep, int *pitch)
{
  ...
  // S_StartSoundAtVolume() - adjust_sound_params(players[displayplayer]->mo, ...);
...
void S_StartSoundAtVolume(void *origin_p, int sfx_id, int volume)
{
  ...
  if (!origin || (origin == players[displayplayer].mo && walkcamera.type < 2)) {
    ...
    if (!adjust_sound_params(players[displayplayer].mo, origin, &volume,
                             &sep, &pitch))
      return;
    else
