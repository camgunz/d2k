...
#endif /* ifdef HAVE_LIBVORBISFILE */
...
static unsigned parsetag(const char *str, int samplerate) {
  ...
  if (seencolon && !seendot) { // HH:MM:SS, but we never converted to samples
...
static const void* vorb_registersong(const void *data, unsigned len) {
  ...
  if (i != 0) {
    D_MsgLocalWarn("vorb_registersong: failed\n");
    return NULL;
  }
  i = ov_test_open(&vf);
...
static void vorb_render_ex(void *dest, unsigned nsamp) {
  ...
  while (nsamp > 0) {
    ...
    if (vorb_looping && vorb_total_pos + nsamp > vorb_loop_from) {
      numread = ov_read_float(&vf,
        &pcmdata,
        vorb_loop_from - vorb_total_pos,
        &bitstreamnum);
    }
    else
#endif    // ZDOOM_AUDIO_LOOP
    numread =  ov_read_float(&vf, &pcmdata, nsamp, &bitstreamnum);
    if (numread == OV_HOLE) { // recoverable error, but discontinue if we get
                              // too many
      localerrors++;
      if (localerrors == 10) {
        D_MsgLocalWarn("vorb_render: many errors.  aborting\n");
        vorb_playing = 0;
    ...
    else if (numread < 0) { // unrecoverable errror
      D_MsgLocalWarn("vorb_render: unrecoverable error\n");
      vorb_playing = 0;
      memset(sout, 0, nsamp * 4);
      return;
    }
    ...
    // volume and downmix
    if (vinfo->channels == 2) {
      for (i = 0; i < numread; i++, sout += 2) { // data is preclipped?
