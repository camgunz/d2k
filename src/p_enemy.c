...
static void P_RecursiveSound(sector_t *sec, int soundblocks,
  ...
  for (i = 0; i < sec->linecount; i++) {
    ...
    other =
      sides[check->sidenum[sides[check->sidenum[0]].sector == sec]].sector;
...
static bool P_CheckMeleeRange(mobj_t *actor) {
...
static bool P_CheckMissileRange(mobj_t *actor) {
  ...
  if (actor->flags & MF_JUSTHIT) { // the target just hit the enemy, so fight
                                   // back!
...
static int P_IsUnderDamage(mobj_t *actor) {
  const struct msecnode_s *secs;
  int dir = 0;

  for (secs = actor->touching_sectorlist; secs; secs = secs->m_tnext) {
    const ceiling_t *cl = secs->m_sector->ceilingdata; // Crushing ceiling

    /*
     *  if ((cl = secs->m_sector->ceilingdata) &&
    ...
    if (cl->thinker.function != T_MoveCeiling) {
      continue;
    }
...
static fixed_t xspeed[8] = {
  FRACUNIT, 47000, 0, -47000, -FRACUNIT, -47000, 0, 47000
};

static fixed_t yspeed[8] = {
...
/* killough 9/12/98 */
static bool P_Move(mobj_t *actor, bool dropoff) {
  ...
  if (friction < ORIG_FRICTION && // sludge
      !(speed = ((
        ORIG_FRICTION_FACTOR - (ORIG_FRICTION_FACTOR - movefactor) / 2
...
static bool P_SmartMove(mobj_t *actor) {
  ...
  if ((on_lift && P_Random(pr_stayonlift) < 230 &&  // Stay on lift
       !P_IsOnLift(actor)) ||
      (tmp_monster_avoid_hazards &&
       !under_damage && // e6y Get away from damage
...
static fixed_t P_AvoidDropoff(mobj_t *actor) {
  int yh = P_GetSafeBlockY(
    (tmbbox[BOXTOP]   = actor->y + actor->radius) - bmaporgy
  );
  ...
  return dropoff_deltax | dropoff_deltay;            // Non-zero if movement
                                                     // prescribed
}
...
static bool P_IsVisible(mobj_t *actor, mobj_t *mo, bool allaround) {
  if (!allaround) {
    angle_t an = R_PointToAngle2(
...
static bool PIT_FindTarget(mobj_t *mo) {
...
static bool P_LookForPlayers(mobj_t *actor, bool allaround) {
  ...
  if (compatibility_level < d2k_compatibility) {
    maxplayers = VANILLA_MAXPLAYERS;
  }
  else {
    maxplayers = MAXPLAYERS;
  }

  if (actor->flags & MF_FRIEND) { // killough 9/9/98: friendly monsters go about
                                  // players differently
    int anyone;
...
static bool P_LookForMonsters(mobj_t *actor, bool allaround) {
  thinker_t *cap, *th;
...
static bool P_HelpFriend(mobj_t *actor) {
  thinker_t *cap, *th;
  ...
  if (actor->flags & MF_FRIEND) {
    cap = &thinkerclasscap[th_friends];
  }
  else {
...
void A_FaceTarget(mobj_t *actor) {
  ...
  actor->angle = R_PointToAngle2(actor->x, actor->y,
    actor->target->x, actor->target->y);
  if (actor->target->flags & MF_SHADOW) { // killough 5/5/98: remove dependence
...
void A_SPosAttack(mobj_t *actor) {
  ...
  for (i = 0; i < 3; i++) { // killough 5/5/98: remove dependence on order of
                            // evaluation:
...
void A_SkelMissile(mobj_t *actor) {
  ...
  A_FaceTarget(actor);
  actor->z += 16 * FRACUNIT;                           // so missile spawns
                                                       // higher
    ...
    for (bx = xl; bx <= xh; bx++) {
      for (by = yl; by <= yh; by++) {
        ...
        if (!P_BlockThingsIterator(bx, by, PIT_VileCheck)) {
          ...
          if (mbf_features) { /* kilough 9/9/98 */
...
static void A_PainShootSkull(mobj_t *actor, angle_t angle) {
  ...
  prestep = 4 * FRACUNIT + 3 *
    (actor->info->radius + mobjinfo[MT_SKULL].radius) / 2;

  x = actor->x + FixedMul(prestep, finecosine[an]);
  y = actor->y + FixedMul(prestep, finesine[an]);
  z = actor->z + 8 * FRACUNIT;

  if (comp[comp_skull]) {                    /* killough 10/98:
...
void A_BossDeath(mobj_t *mo) {
  ...
  if (gamemode == commercial) {
    ...
    if (comp[comp_666] && gameepisode < 4) {
      ...
      // instead of separate checks for each episode in doomult.exe,
      // plutonia.exe and tnt.exe
...
void P_SpawnBrainTargets(void) {// killough 3/26/98: renamed old function
  ...
  for (thinker = thinkercap.next;
    ...
    if (thinker->function == P_MobjThinker) {
      ...
      if (m->type == MT_BOSSTARGET) { // killough 2/7/98: remove limit on icon
                                      // landings:
...
void A_BrainExplode(mobj_t *mo) { // killough 5/5/98: remove dependence on order
                                  // of evaluation:
...
void A_BrainSpit(mobj_t *mo) {
  ...
  if (!newmobj || !newmobj->state || newmobj->momy == 0 ||
  ...
  newmobj->reactiontime =
    (short)(((targ->y - mo->y) / newmobj->momy) / newmobj->state->tics);
...
void A_SpawnFly(mobj_t *mo) {
  ...
  // e6y: monsters spawned by Icon of Sin should not be countable for total
  // killed.
...
void A_PlayerScream(mobj_t *mo) {
  ...
  if (gamemode != shareware && mo->health < -50) {
    sound = sfx_pdiehi;                                 // IF THE PLAYER DIES
                                                        // LESS THAN -50%
                                                        // WITHOUT GIBBING
  }
...
void A_Mushroom(mobj_t *actor) {
  ...
  bool mbf = (compatibility_level == mbf_compatibility &&
  ...
  A_Explode(actor);                                   // First make normal
                                                      // explosion
  ...
  for (i = -n; i <= n; i += 8) {
    for (j = -n; j <= n; j += 8) {
      ...
      mo->flags &= ~MF_NOGRAVITY;                     // Make debris fall under
                                                      // gravity
    }
  }
}
