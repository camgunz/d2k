...
function RetractableTextWidget:set_vertical_alignment(vertical_alignment)
    ...
    if vertical_alignment ~= TextWidget.ALIGN_BOTTOM then
      error([[
        Retractable text widgets can currently only be vertically aligned to
        the bottom.
      ]])
    end
...
function RetractableTextWidget:update_visible_line_indices()
    ...
    -- first_visible_line_index negative.  last_retracted_line_index will be 0
    -- to start, and as a result this check ensures first_visible_line_index
    ...
    -- If the new set of visible line indices is longer than
    -- display_line_count, then remove old lines until it isn't.
...
function RetractableTextWidget:tick()
    ...
    elseif self:get_retraction_start_time() == 0 then
        self:set_retraction_start_time(
            current_ms + self:get_retraction_timeout()
        )
