...
function InputWidget:render()
    ...
    local min_visible_cursor_x = self:get_scroll_offset()
    local max_visible_cursor_x = (text_width - prompt_width) + min_cursor_x

    if cursor_pos.x > max_visible_cursor_x then
        self:set_scroll_offset(cursor_pos.x - (text_width - prompt_width))
    elseif cursor_pos.x < min_visible_cursor_x then
...
function InputWidget:print_cursor_stats(s)
    if DEBUG then
        local cursor_position = self:get_layout():index_to_pos(
            self:get_cursor()
        )
